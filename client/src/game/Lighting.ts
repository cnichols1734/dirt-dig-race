...
import { SCALED_TILE, ORE_COLORS, BIOME_LIGHT_COLORS } from '../utils/constants';
import { getBiome } from '../utils/helpers';
...
export class LightingSystem {
  ...
  updateLantern(px: number, py: number, radius: number) {
    ...
    const r = (radius + 1) * SCALED_TILE;

    const flicker = 0.85 + Math.sin(this.lanternFlicker) * 0.15;

    for (let i = 5; i >= 0; i--) {
      const ratio = i / 5;
      const cr = r * (0.3 + ratio * 0.7);
      const alpha = (1 - ratio) * 0.25 * flicker;
  ...
  flashBreak(wx: number, wy: number) {
    this.hitFlash.clear();
    this.hitFlash.circle(wx, wy, SCALED_TILE * 1.5);
    this.hitFlash.fill({ color: 0xFFB347, alpha: 0.5 });
    this.hitFlash.alpha = 1;
  ...
  updateOreGlows(tiles: TileData[][], revealedSet: Set<string>) {
    ...
    for (const key of revealedSet) {
      ...
      this.oreGlows.fill({ color, alpha: 0.08 });
    }
  }

  update(dt: number) {
    this.lanternFlicker += dt * 0.008;

    if (this.hitFlashTimer > 0) {
      this.hitFlashTimer -= dt;
      this.hitFlash.alpha = Math.max(0, this.hitFlashTimer / 80);
    }
  }
}
